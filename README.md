...
- Cloudflare Workflow `AGENT_TASK_WORKFLOW`: 运行 Agent 任务队列里的长期任务和未来任务；未绑定时会 fallback 到 Durable Object alarm / `waitUntil`。
...
Cloudflare Workflows 已包含在 Workers Free 和 Paid 计划中，免费计划下与 Workers 请求共享每天 100,000 次请求额度，并包含 1GB Workflow state。仓库默认开启 `TELEGRAM_UPDATE_WORKFLOW` 和 `AGENT_TASK_WORKFLOW` 绑定；如果部署环境没有对应绑定，Telegram webhook 会回退到原来的 `waitUntil` 后台处理，Agent 任务队列会回退到 Durable Object alarm / `waitUntil`。
