...
- [x] `send_media_group` 节点 - 发送多媒体组（图片/视频混合）
  - [ ] 媒体组条目支持独立 `parse_mode` / `caption`
  - [ ] 媒体组支持音频/文档类型
  - [ ] 媒体组数量上限（10）在 WebUI 提示
...
### 消息功能增强
- [ ] `send_audio` 节点 - 发送音频文件
- [ ] `send_animation` 节点 - 发送 GIF 动画
...
### 测试
- [ ] 添加单元测试（节点 handler 测试）
- [ ] 添加集成测试（API 端点测试）
- [ ] 添加 E2E 测试（模拟 Telegram Update）
...
### 监控与日志（当前完全缺失）
- [ ] 添加结构化日志系统（可选：pino / consola）
- [ ] 工作流执行时记录每个节点的输入输出
- [ ] 记录节点执行耗时
- [ ] 添加执行历史存储（最近 N 次执行记录）
- [ ] WebUI 添加「执行日志」页面查看历史
- [ ] 添加错误追踪（可选 Sentry / Logflare）
- [ ] 添加执行统计（次数、成功率、平均耗时）

### 调试功能（当前完全缺失）
- [ ] 添加调试模式开关（环境变量 / WebUI 配置）
- [ ] 调试模式下保存完整执行上下文
- [ ] 工作流失败时保存错误现场（节点 ID、输入、堆栈）
- [ ] WebUI 添加「工作流调试器」：单步执行/断点
- [ ] 添加「模拟执行」：不实际发送消息，仅展示结果
- [ ] 添加节点执行预览（输入渲染后的参数）
