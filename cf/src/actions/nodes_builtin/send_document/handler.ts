...
export const handler: ActionHandler = async (params, context) => {
  ...
  const documentSource = String(params.document_source || params.file_source || "");
  if (!documentSource) {
  ...
  const r2File = await loadR2File(context.env as any, documentSource);
  if (r2File) {
    ...
    payload.append("document", r2File.blob, filename);
    const result = await callTelegramForm(context.env as any, "sendDocument", payload);
  ...
  const result = await callTelegram(context.env as any, "sendDocument", payload);
  return { message_id: (result.result as any)?.message_id };
};
