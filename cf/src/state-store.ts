...
export class StateStore implements DurableObject {
  ...
  private normalizeAgentDocument(
    ...
    return this.agentState().normalizeDocument(rawKey, rawDoc, fallback, now);
  }

  private async loadAgentConfig(): Promise<AgentConfig> {
    return await this.agentState().loadConfig();
  }

  private async saveAgentConfig(config: AgentConfig): Promise<void> {
    await this.agentState().saveConfig(config);
  }
  ...
  private normalizeAgentChatHistory(input: unknown): AgentChatMessage[] {
    return this.agentState().normalizeChatHistory(input);
  }

  private normalizeAgentSessionId(input: unknown): string {
    return this.agentState().normalizeSessionId(input);
  }
  ...
  private agentTasks(): AgentTaskService {
    return new AgentTaskService({
      ...
      workflow: this.env.AGENT_TASK_WORKFLOW,
      normalizeChatHistory: (input) => this.normalizeAgentChatHistory(input),
      normalizeSessionId: (input) => this.normalizeAgentSessionId(input),
      normalizeRuntimePayload: (input) => this.normalizeAgentRuntimePayload(input),
      compactLogValue: (value, limit) => this.compactAgentLogValue(value, limit),
      sanitizeErrorMessage: (error) => this.sanitizeAgentErrorMessage(error),
      handleAgentChat: (request) => this.handleAgentChat(request),
      sendText: (chatId, text, parseMode) => this.sendText(chatId, text, parseMode),
    });
  }

  private normalizeAgentTaskId(input: unknown): string {
    return normalizeAgentTaskIdValue(input);
  }

  private async runDueAgentTasks(runner: string): Promise<void> {
    await this.agentTasks().runDue(runner);
  }

  private async updateAgentTaskHeartbeat(
    taskId: string,
    patch: { current_step?: string; progress?: number } = {}
  ): Promise<void> {
    await this.agentTasks().updateHeartbeat(taskId, patch);
  }

  private async handleAgentTasksList(url: URL): Promise<Response> {
    return await this.agentTasks().handleList(url);
  }

  private async handleAgentTaskCreate(request: Request): Promise<Response> {
    return await this.agentTasks().handleCreate(request);
  }

  private async handleAgentTaskGet(taskId: string): Promise<Response> {
    return await this.agentTasks().handleGet(taskId);
  }

  private async handleAgentTaskCancel(taskId: string): Promise<Response> {
    return await this.agentTasks().handleCancel(taskId);
  }

  private async handleAgentTaskRetry(taskId: string): Promise<Response> {
    return await this.agentTasks().handleRetry(taskId);
  }

  private async handleInternalTaskRun(request: Request): Promise<Response> {
    return await this.agentTasks().handleInternalRun(request);
  }
