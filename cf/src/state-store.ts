...
export class StateStore implements DurableObject {
  ...
  private buildWorkflowNodesSkillMarkdown(
    ...
    const toolIds = tools.map((tool) => String(tool.id || "").trim()).filter(Boolean);
    const categoryIndex = categoryGroups.map((group) => {
      const ids = group.tools.map((tool) => String(tool.id || "").trim()).filter(Boolean);
      return [
        `### ${group.key}`,
        "",
        `Use this folder when the task needs ${group.key} workflow nodes.`,
        "",
        ...ids.map((id) => `- \`tools/${group.key}/${id}.md\` -> \`${id}\``),
      ].join("\n");
    });
    return [
      "---",
      ...
      "# Workflow Node Tools",
      "",
      "This is the root skill for all workflow node tools. Load this skill when an agent needs to inspect or operate the Telegram bot workflow engine.",
      "",
      "## How To Read This Skill",
      "",
      "1. Read this `SKILL.md` first to choose the right virtual folder.",
      "2. Do not load every tool schema by default. Pick the smallest relevant category first.",
      "3. Read `tools/<category>/<node_id>.md` only for tools that may be used.",
      "4. Before calling a tool with side effects, verify required IDs, user intent, and whether the workflow context permits the action.",
      "5. Prefer composing a short plan before calling multiple tools. The control/order of execution belongs to the workflow engine; tool calls should respect node input/output contracts.",
      "",
      "## When To Use",
      "",
      "- Use this skill to let an agent build, inspect, explain, or execute workflow-node operations.",
      "- Use it when the user asks for Telegram message handling, workflow branching, data transformation, LLM generation, R2 caching, or Telegram admin operations.",
      "- Do not use it for unrelated general chat or tasks that do not require workflow nodes.",
      "",
      "## Tool Selection Rules",
      "",
      "- `trigger`: choose only when defining how a workflow starts.",
      "- `flow`: choose for branching, loops, sub-workflows, delay, and error handling.",
      "- `message`: choose for sending, editing, copying, forwarding, deleting, or updating Telegram messages/media.",
      "- `telegram`: choose for chat/member lookup, user input waits, payments, shipping, and admin actions.",
      "- `navigation`: choose for menu/button redirection.",
      "- `ai`: choose when text or JSON must be generated by a configured LLM.",
      "- `data`: choose for math, array, and date/time operations.",
      "- `integration`: choose for external/R2 file caching.",
      "- `utility`: choose for string, JSON, variables, placeholders, and existing IDs.",
      "",
      "## Safety Rules",
      "",
      "- Treat `side_effect`, `network`, and `high` risk tools as non-idempotent unless the tool document says otherwise.",
      "- Telegram admin/payment tools require explicit workflow intent or user confirmation.",
      "- Never invent `chat_id`, `message_id`, `user_id`, workflow IDs, or model IDs. Read them from context, references, or existing ID tools.",
      "- If required inputs are missing, ask for them or use a safe lookup/provide node instead of guessing.",
      "",
      ...
      "## Output Contract",
      "",
      "After selecting tools, return the planned tool calls and the expected node outputs. If building a workflow, keep data passing through references and use edges only for execution order.",
    ].join("\n");
  }
    ...
    return [
      {
        ...
        label: "Workflow Node Tools",
        category: "workflow",
        description: "Root skill for all workflow node tools. Category folders are represented inside SKILL.md.",
        ...
        virtual_folders: categoryGroups.map((group) => ({
          path: `generated/workflow_nodes/tools/${group.key}`,
          category: group.key,
          tool_ids: group.tools.map((tool) => String(tool.id || "")).filter(Boolean),
        })),
        content_md: this.buildWorkflowNodesSkillMarkdown(sortedTools, categoryGroups),
        format: "markdown",
        source: "generated",
      },
    ];
