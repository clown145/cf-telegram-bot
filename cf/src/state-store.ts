...
export class StateStore implements DurableObject {
  ...
  async alarm(): Promise<void> {
    await this.runDueAgentTasks("alarm");
  }
  ...
  private agentTaskKey(taskId: string): string {
    return `agent_task:${taskId}`;
  }

  private normalizeAgentTaskId(input: unknown): string {
    return String(input || "")
      .trim()
      .replace(/[^a-zA-Z0-9:._-]+/g, "-")
      .replace(/-+/g, "-")
  ...
  private normalizeAgentTaskScheduledAt(input: unknown): number {
  ...
  private normalizeAgentTaskRecord(raw: unknown): AgentTaskRecord | null {
    const source = raw && typeof raw === "object" && !Array.isArray(raw) ? (raw as Record<string, unknown>) : {};
    const id = this.normalizeAgentTaskId(source.id);
    ...
    const status: AgentTaskStatus = ["queued", "waiting", "running", "succeeded", "failed", "cancelled", "stalled"].includes(statusRaw)
    ...
    return {
      ...
      input: {
        message,
        history: this.normalizeAgentChatHistory(input.history),
        session_id: this.normalizeAgentSessionId(input.session_id),
        runtime_context: this.normalizeAgentRuntimePayload(input.runtime_context || input.runtime),
      ...
      result: source.result && typeof source.result === "object" && !Array.isArray(source.result)
  ...
  private publicAgentTask(task: AgentTaskRecord): Record<string, unknown> {
    return {
      id: task.id,
      type: task.type,
      status: task.status,
  ...
  private async loadAgentTask(taskId: string): Promise<AgentTaskRecord | null> {
    const id = this.normalizeAgentTaskId(taskId);
    if (!id) {
      return null;
    }
    return this.normalizeAgentTaskRecord(await this.state.storage.get<AgentTaskRecord>(this.agentTaskKey(id)));
  }

  private async saveAgentTask(task: AgentTaskRecord): Promise<void> {
    task.updated_at = Date.now();
    await this.state.storage.put(this.agentTaskKey(task.id), task);
  }

  private appendAgentTaskLog(
    ...
    task.logs = [
      ...(task.logs || []),
      {
        ...
        data: data === undefined ? undefined : this.compactAgentLogValue(data, 6000),
      },
    ].slice(-200);
    return task;
  }

  private async listAgentTasksRaw(): Promise<AgentTaskRecord[]> {
    const listed = await this.state.storage.list<AgentTaskRecord>({ prefix: "agent_task:" });
    return Array.from(listed.values())
      .map((entry) => this.normalizeAgentTaskRecord(entry))
      .filter((entry): entry is AgentTaskRecord => Boolean(entry))
      .sort((a, b) => b.created_at - a.created_at);
  }

  private async markStalledAgentTasks(): Promise<number> {
    const now = Date.now();
    let changed = 0;
    const tasks = await this.listAgentTasksRaw();
    for (const task of tasks) {
      ...
      this.appendAgentTaskLog(task, "warn", "Marked stalled because heartbeat expired.");
      await this.saveAgentTask(task);
      changed += 1;
    }
    return changed;
  }

  private async scheduleNextAgentTaskAlarm(): Promise<void> {
    const storage = this.state.storage as unknown as {
    ...
    if (next) {
      await storage.setAlarm(Math.max(Date.now() + 1000, next.scheduled_at));
    } else if (typeof storage.deleteAlarm === "function") {
      await storage.deleteAlarm();
    }
  }

  private async updateAgentTaskHeartbeat(
    taskId: string,
    patch: Partial<Pick<AgentTaskRecord, "current_step" | "progress">> = {}
  ): Promise<void> {
    const task = await this.loadAgentTask(taskId);
  ...
  private async enqueueAgentTask(task: AgentTaskRecord): Promise<{ mode: string; workflow_instance_id?: string; error?: string }> {
    ...
    if (binding && typeof binding.create === "function") {
      ...
      try {
        ...
        this.appendAgentTaskLog(task, "warn", "Cloudflare Workflow enqueue failed; falling back to Durable Object.", String(error));
        await this.saveAgentTask(task);
      }
    }
    ...
    if (task.scheduled_at > Date.now()) {
      await this.scheduleNextAgentTaskAlarm();
      return { mode: "durable_object_alarm" };
    }
    this.state.waitUntil(this.runAgentTask(task.id, "waitUntil"));
    return { mode: "waitUntil" };
  }

  private async createAgentTaskFromPayload(payload: Record<string, unknown>): Promise<{
    task?: AgentTaskRecord;
    ...
    const now = Date.now();
    const scheduledAt = this.normalizeAgentTaskScheduledAt(payload.scheduled_at || payload.run_at);
    const runtimeContext = this.normalizeAgentRuntimePayload(payload.runtime_context || payload.runtime);
    const task: AgentTaskRecord = {
      id: this.normalizeAgentTaskId(payload.id) || generateId(AGENT_TASK_ID_PREFIX),
      type: "agent_chat",
      ...
      input: {
        message,
        history: this.normalizeAgentChatHistory(payload.history),
        session_id: this.normalizeAgentSessionId(payload.session_id),
        runtime_context: runtimeContext,
        model_id: String(payload.model_id || "").trim(),
        notify_chat_id: String(payload.notify_chat_id || (runtimeContext as any).chat_id || "").trim(),
      },
    ...
    this.appendAgentTaskLog(task, "info", task.status === "waiting" ? "Task scheduled." : "Task queued.");
    await this.saveAgentTask(task);
    const enqueue = await this.enqueueAgentTask(task);
    return { task: (await this.loadAgentTask(task.id)) || task, enqueue };
  }

  private async runDueAgentTasks(runner: string): Promise<void> {
    await this.markStalledAgentTasks();
    const now = Date.now();
    const tasks = await this.listAgentTasksRaw();
    for (const task of tasks) {
      ...
      this.state.waitUntil(this.runAgentTask(task.id, runner));
    }
    await this.scheduleNextAgentTaskAlarm();
  }

  private async runAgentTask(taskId: string, runner: string): Promise<Record<string, unknown>> {
    let task = await this.loadAgentTask(taskId);
    if (!task) {
    ...
    if (task.cancel_requested || task.status === "cancelled") {
      ...
      task.finished_at = now;
      this.appendAgentTaskLog(task, "info", "Task was cancelled before execution.");
      await this.saveAgentTask(task);
    ...
    task.progress = 0.05;
    task.error = "";
    this.appendAgentTaskLog(task, "info", "Task started.", { runner, attempt: task.attempt });
    await this.saveAgentTask(task);
    ...
    try {
      ...
      task = (await this.loadAgentTask(task.id)) || task;
      if (task.cancel_requested) {
        task.status = "cancelled";
        task.finished_at = Date.now();
        this.appendAgentTaskLog(task, "info", "Task was cancelled after agent returned.");
        await this.saveAgentTask(task);
      ...
      this.appendAgentTaskLog(task, "info", "Task succeeded.");
      await this.saveAgentTask(task);
      if (task.input.notify_chat_id && task.result_message) {
        await this.sendText(task.input.notify_chat_id, task.result_message.slice(0, 4096), undefined).catch((error) => {
          console.error("send task result failed:", error);
        });
      }
      return { ok: true, status: task.status, task: this.publicAgentTask(task) };
    } catch (error) {
      task = (await this.loadAgentTask(task.id)) || task;
      task.status = task.attempt < task.max_attempts ? "queued" : "failed";
      task.finished_at = task.status === "failed" ? Date.now() : undefined;
      task.heartbeat_at = Date.now();
      task.error = this.sanitizeAgentErrorMessage(error);
      this.appendAgentTaskLog(task, "error", task.error);
      await this.saveAgentTask(task);
      ...
      if (task.input.notify_chat_id) {
        await this.sendText(task.input.notify_chat_id, `Agent 任务失败: ${task.error}`, undefined).catch((sendError) => {
  ...
  private async handleAgentTasksList(url: URL): Promise<Response> {
    await this.markStalledAgentTasks();
    ...
    const tasks = (await this.listAgentTasksRaw())
      ...
      .slice(0, limit)
      .map((task) => this.publicAgentTask(task));
    return jsonResponse({ status: "ok", tasks, total: tasks.length });
  }

  private async handleAgentTaskCreate(request: Request): Promise<Response> {
    let payload: Record<string, unknown>;
    try {
      payload = await parseJson<Record<string, unknown>>(request);
    } catch (error) {
    ...
    const result = await this.createAgentTaskFromPayload(payload);
    if (result.error || !result.task) {
      return jsonResponse({ error: result.error || "create task failed" }, result.status || 400);
    }
    return jsonResponse({
      status: "ok",
      task: this.publicAgentTask(result.task),
      enqueue: result.enqueue || {},
    });
  }

  private async handleAgentTaskGet(taskId: string): Promise<Response> {
    const task = await this.loadAgentTask(taskId);
    if (!task) {
      return jsonResponse({ error: `task not found: ${taskId}` }, 404);
    }
    return jsonResponse({ status: "ok", task: this.publicAgentTask(task) });
  }

  private async handleAgentTaskCancel(taskId: string): Promise<Response> {
    const task = await this.loadAgentTask(taskId);
    if (!task) {
      return jsonResponse({ error: `task not found: ${taskId}` }, 404);
    }
    ...
    this.appendAgentTaskLog(task, "warn", "Cancellation requested.");
    await this.saveAgentTask(task);
    await this.scheduleNextAgentTaskAlarm();
    return jsonResponse({ status: "ok", task: this.publicAgentTask(task) });
  }

  private async handleAgentTaskRetry(taskId: string): Promise<Response> {
    const task = await this.loadAgentTask(taskId);
    if (!task) {
      return jsonResponse({ error: `task not found: ${taskId}` }, 404);
    }
    ...
    task.progress = 0;
    this.appendAgentTaskLog(task, "info", "Task retry requested.");
    await this.saveAgentTask(task);
    const enqueue = await this.enqueueAgentTask(task);
    return jsonResponse({ status: "ok", task: this.publicAgentTask((await this.loadAgentTask(task.id)) || task), enqueue });
  }

  private async handleInternalTaskRun(request: Request): Promise<Response> {
    ...
    const taskId = this.normalizeAgentTaskId(payload.task_id || payload.id);
    if (!taskId) {
      return jsonResponse({ error: "task_id is required" }, 400);
    }
    const result = await this.runAgentTask(taskId, "cloudflare_workflows");
  ...
  private async handleAgentChat(request: Request): Promise<Response> {
    ...
    const taskId = this.normalizeAgentTaskId(payload.task_id);
    ...
    try {
      for (let step = 0; step < maxSteps; step += 1) {
        if (taskId) {
          await this.updateAgentTaskHeartbeat(taskId, {
            current_step: `llm_turn_${step + 1}`,
            progress: Math.min(0.9, 0.1 + step / Math.max(maxSteps, 1) * 0.7),
          });
        }
        ...
        for (const toolCall of result.tool_calls) {
          if (taskId) {
            await this.updateAgentTaskHeartbeat(taskId, {
              current_step: `tool:${toolCall.name}`,
              progress: Math.min(0.95, 0.15 + step / Math.max(maxSteps, 1) * 0.75),
            });
          }
          ...
          if (taskId) {
            await this.updateAgentTaskHeartbeat(taskId, {
              current_step: `tool_done:${toolCall.name}`,
              progress: Math.min(0.95, 0.2 + step / Math.max(maxSteps, 1) * 0.75),
            });
          }
        }
      }
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    if (path === "/internal/tasks/run" && method === "POST") {
      return await this.handleInternalTaskRun(request);
    }
    ...
    if (path === "/api/tasks") {
      if (method === "GET") {
        return await this.handleAgentTasksList(url);
      }
      if (method === "POST") {
        return await this.handleAgentTaskCreate(request);
      }
    }
    ...
    if (path.startsWith("/api/tasks/")) {
      ...
      if (parts[1] === "cancel" && method === "POST") {
        return await this.handleAgentTaskCancel(taskId);
      }
      if (parts[1] === "retry" && method === "POST") {
        return await this.handleAgentTaskRetry(taskId);
      }
      if (!parts[1] && method === "GET") {
        return await this.handleAgentTaskGet(taskId);
      }
    }
