...
const iconAgentTasks =
  "M5 3h10l4 4v14H5z M14 4v4h4 M8 10h8v2H8z M8 14h6v2H8z M8 18h4v2H8z";
...
const menuOptions = computed<MenuOption[]>(() => [
  ...
  { label: t("app.tabs.agentTasks"), key: "agentTasks", icon: renderMenuIcon(iconAgentTasks) },
...
const handleMenuUpdate = (key: string) => {
  ...
  } else if (key === "agentTasks") {
    router.push({ name: "agentTasks" });
  } else if (key === "logs") {
...
onMounted(async () => {
  ...
  if (route.path === "/" && route.name === "buttons") {
    ...
    if (
      lastTab &&
      lastTab !== "buttons" &&
      ["workflow", "bot", "llm", "mcp", "skills", "agent", "agentSessions", "agentTasks", "logs"].includes(lastTab)
    ) {
